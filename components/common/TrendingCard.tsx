...
const TrendingCard = ({ name, imageUrl }: TrendingCardProps) => {
  ...
  return (
    // @ts-ignore
    <div style={{ '--image-url': `url(${imageUrl})` }} className={`group relative cursor-pointer overflow-hidden bg-[image:var(--image-url)] aspect-square h-full w-full bg-[length:100%_100%] hover:bg-[length:120%_120%] transition-all duration-500 ease-in-out bg-center bg-no-repeat rounded-sm`}>
