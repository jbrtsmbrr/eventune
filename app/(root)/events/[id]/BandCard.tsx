...
const BandCard = ({ bandName, imageUrl, trendingNumber = 0 }: IBandCardProps) => {
  ...
  return (
    <div className='relative h-full group overflow-hidden min-h-[400px] rounded-sm'>
      ...
      <div className='absolute bottom-0 h-fit w-full flex flex-col justify-center items-center text-center gap-2 z-10 translate-y-14 group-hover:-translate-y-14 transition-transform duration-500 ease-in-out'>
        ...
        <button className='mt-8 text-xs border-purple-300 border py-2 px-4 text-purple-100 hover:bg-purple-400 hover:bg-opacity-20 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out delay-200 uppercase tracking-wide'>Visit Artist</button>
      </div>
    </div>
  )
}
